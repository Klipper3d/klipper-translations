...
Los campos de este documento están sujetos a cambios. Si utiliza un atributo, asegúrese de revisar el [documento de cambios de configuración](Config_Changes.md) al actualizar el software Klipper.
...
La siguiente información está disponible en el objeto [bed_mesh](Config_Reference.md#bed_mesh):

- `profile_name`, `mesh_min`, `mesh_max`, `probed_matrix`, `mesh_matrix`: Información sobre el bed_mesh actualmente activo.
...
La siguiente información está disponible en el objeto `configfile` (este objeto siempre está disponible):
