...
El primer paso en la calibración es determinar el DRIVE_CURRENT adecuado para el sensor. Coloque la impresora en su posición inicial y navegue con el cabezal de herramientas de modo que el sensor quede cerca del centro de la cama y a unos 20 mm por encima de ella. A continuación, ejecute el comando «LDC_CALIBRATE_DRIVE_CURRENT CHIP=<config_name>». Por ejemplo, si la sección de configuración se denominó «[probe_eddy_current my_eddy_probe]», se ejecutaría «LDC_CALIBRATE_DRIVE_CURRENT CHIP=my_eddy_probe». Este comando debería completarse en unos segundos. Una vez completado, ejecute el comando «SAVE_CONFIG» para guardar los resultados en printer.cfg y reinicie.
