...
* Klipper es compatible con impresoras con múltiples microcontroladores. Por ejemplo, un microcontrolador podría utilizarse para controlar un extrusor, mientras que otro controla los calentadores de la impresora y un tercero controla el resto de la impresora. El software host de Klipper implementa la sincronización del reloj para tener en cuenta la deriva del reloj entre los microcontroladores. No se necesita ningún código especial para habilitar múltiples microcontroladores, solo se requieren unas pocas líneas adicionales en el archivo de configuración.
* Configuración mediante un sencillo archivo de configuración. No es necesario volver a flashear el microcontrolador para cambiar un ajuste. Toda la configuración de Klipper se almacena en un archivo de configuración estándar que se puede editar fácilmente. Esto facilita la configuración y el mantenimiento del hardware.
* Klipper es compatible con «Smooth Pressure Advance», un mecanismo que tiene en cuenta los efectos de la presión dentro de un extrusor. Esto reduce el «goteo» del extrusor y mejora la calidad de las esquinas impresas. La implementación de Klipper no introduce cambios instantáneos en la velocidad del extrusor, lo que mejora la estabilidad y la robustez generales.
* Klipper admite la función «Input Shaping» (modelado de entrada) para reducir el impacto de las vibraciones en la calidad de impresión. Esto puede reducir o eliminar el «ringing» (también conocido como «ghosting», «echoing» o «rippling») en las impresiones. También puede permitir obtener velocidades de impresión más rápidas sin perder la alta calidad de impresión.
* Klipper utiliza un «solucionador iterativo» para calcular tiempos de paso precisos a partir de ecuaciones cinemáticas simples. Esto facilita la adaptación de Klipper a nuevos tipos de robots y mantiene la precisión de los tiempos incluso con cinemáticas complejas (no se necesita «segmentación de líneas»).
