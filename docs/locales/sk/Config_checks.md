...
Tento dokument poskytuje zoznam krokov, ktoré pomáhajú potvrdiť nastavenia pinov v konfiguračnom súbore Klipperu printer.cfg. Je dobrý nápad spustiť tieto kroky po nasledujúcich krokoch v [inštalačný dokument](Inštalácia.md).

V rámci tejto príručky môže byť potrebné vykonať zmeny v konfiguračnom súbore Klipper. Uistite sa, že po každej zmene konfiguračného súboru použijete príkaz RESTART, aby ste sa uistili, že sa zmena prejaví (na karte Octoprint terminal napíšte „restart“ a potom kliknite na „Odoslať“). Je tiež dobré po každom REŠTARTE spustiť príkaz STATUS, aby ste si overili, či sa konfiguračný súbor úspešne načítal.
...
## Skontrolovať aktivačný pin krokového motora
...
Ak sa koncová poloha vôbec nemení, potom to vo všeobecnosti znamená, že koncová spínač je pripojený k inému pinu. Môže to však vyžadovať aj zmenu nastavenia pullup pinu (znak '^' na začiatku názvu endstop_pin - väčšina tlačiarní bude používať pullup rezistor a znak '^' by mal byť prítomný).

## Skontrolovať krokové motory
...
Po overení všetkých koncových dorazov a krokových motorov by sa mal otestovať mechanizmus navádzania do východiskovej polohy. Vydaním príkazu G28 na nastavenie všetkých osí do východiskovej polohy. Ak sa tlačiareň nenastaví správne do východiskovej polohy, odpojte napájanie. V prípade potreby znova spustite kroky overenia koncových dorazov a krokového motora.
...
Táto príručka má pomôcť so základným overením nastavení pinov v konfiguračnom súbore Klipper. Nezabudnite si prečítať príručku [vyrovnávanie lôžka](Bed_Level.md). Informácie o konfigurácii krájača pomocou Klipperu nájdete aj v dokumente [Slicers](Slicers.md).

Po overení funkčnosti základnej tlače je vhodné zvážiť kalibráciu [presunu tlaku](Pressure_Advance.md).
