...
Ці інструкції передбачають, що програмне забезпечення працюватиме на хості на базі Linux з інтерфейсом, сумісним з Klipper. Рекомендується використовувати як хост-машину SBC (малий платний комп'ютер), такий як Raspberry Pi або пристрій Linux на базі Debian (див. інші варіанти в розділі [FAQ](FAQ.md#can-i-run-klipper-on-something-other-than-a-raspberry-pi-3)).

Для цілей цих інструкцій host стосується пристрою Linux, а mcu — плати принтера. SBC стосується терміна «малий платовий комп’ютер», такого як Raspberry Pi.
...
Існує багато способів отримати образ ОС для Klipper для використання SBC, більшість із них залежать від того, який інтерфейс ви хочете використовувати. Деякі виробники цих плат SBC також надають власні образи, орієнтовані на Klipper.

Два основні фронтенди на базі Moonraker — це [Fluidd](https://docs.fluidd.xyz/) та [Mainsail](https://docs.mainsail.xyz/), останній з яких має попередньо створений інсталяційний образ ["MainsailOS"](https://docs-os.mainsail.xyz/), який має опцію для Raspberry Pi та деяких варіантів OrangePi.
...
Зазвичай ви починаєте з базового образу для вашого SBC, наприклад, RPiOS Lite, або, у випадку пристрою x86 Linux, Ubuntu Server. Зверніть увагу, що варіанти для робочого столу не рекомендуються через певні допоміжні програми, які можуть перешкоджати роботі деяких функцій Klipper і навіть маскувати доступ до деяких плат принтера.

KIAUH можна використовувати для встановлення Klipper та пов'язаних з ним програм на різноманітні системи на базі Linux, які працюють під керуванням Debian. Більше інформації можна знайти за адресою https://github.com/dw-0/kiauh
...
Якщо коментарі у верхній частині [файлу конфігурації принтера](#obtain-a-klipper-configuration-file) описують спеціальні кроки для «завантаження» остаточного зображення на плату керування принтером, виконайте ці кроки, а потім перейдіть до [налаштування OctoPrint](#configuring-octoprint-to-use-klipper).
...
Для поширених мікроконтролерів з STM32 або клонованими мікросхемами, мікросхемами LPC та іншими, зазвичай потрібна початкова прошивка Klipper через SD-карту.

Під час прошивки цим методом важливо переконатися, що друкована плата не підключена до хоста через USB, оскільки деякі плати можуть подавати живлення назад на плату та запобігати прошивці.
...
Можливо, найпростіший спосіб налаштувати файл конфігурації Klipper – це використовувати вбудовані редактори Mainsail або Fluidd. Вони дозволять користувачеві відкривати приклади конфігурації та зберігати їх у форматі printer.cfg.
...
Або ж можна скопіювати та редагувати файл безпосередньо на хості через SSH. Це може виглядати приблизно так (обов’язково оновіть команду, щоб використовувати відповідне ім’я файлу конфігурації принтера):
...
Після створення та редагування файлу необхідно буде виконати команду "restart" у командній консолі, щоб завантажити конфігурацію. Команда "status" повідомить про готовність принтера, якщо конфігураційний файл Klipper успішно прочитано, а мікроконтролер успішно знайдено та налаштовано.
...
Klipper повідомляє про помилки через командну консоль та спливаючі вікна у Fluidd та Mainsail. Команду "status" можна використовувати для повторного повідомлення про помилки. Журнал доступний і зазвичай знаходиться за адресою `~/printer_data/logs/klippy.log`.
