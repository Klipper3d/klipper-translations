...
* Підтримка автоматичного вирівнювання платформи. Klipper можна налаштувати для базового виявлення нахилу платформи або для повного вирівнювання сітки платформи. Сітку платформи можна налаштувати відповідно до розміру друку (адаптивна сітка платформи). Якщо платформа використовує кілька Z-крокових датчиків, Klipper також може вирівнювати, незалежно маніпулюючи Z-кроковими датчиками. Підтримується більшість датчиків висоти Z, включаючи датчики BL-Touch та датчики з сервоприводом. Датчики можна калібрувати для компенсації скручування осей. Якщо використовується "вихрострумовий зонд", можна використовувати швидке сканування сітки платформи
...
* Підтримка поширених датчиків температури (наприклад, поширених термісторів, AD595, AD597, AD849x, PT100, PT1000, MAX6675, MAX31855, MAX31856, MAX31865, BME280, HTU21D, DS18B20, AHT10, SHT3x та LM75). Також можна налаштувати власні термістори та аналогові датчики температури. Можна контролювати внутрішній датчик температури мікроконтролера та внутрішній датчик температури Raspberry Pi.
* Базовий захист термонагрівача, включений за замовчуванням.
* Підтримка стандартних вентиляторів, вентиляторів з соплами та вентиляторів з контролем температури. Немає потреби тримати вентилятори увімкненими, коли принтер не працює. Швидкість вентилятора можна контролювати на вентиляторах з тахометром. Вентилятору можна призначити «математичну формулу» для автоматичного оновлення швидкості вентилятора.
...
* Підтримка вимірювання та запису прискорення за допомогою акселерометрів adxl345, mpu9250, mpu6050, lis2dw12, lis3dh та icm20948.
