...
[вентилатор]
щифт:
# Изходен щифт, контролиращ вентилатора. Този параметър трябва да бъде предоставен.
#max_power: 1.0
# Максималната мощност (изразена като стойност от 0,0 до 1,0), която
# щифт може да е настроен на. Стойността 1.0 позволява щифтът да бъде зададен изцяло
# разрешено за удължени периоди, докато стойност 0,5 би позволила
# щифт да бъде активиран за не повече от половината време. Тази настройка може да
# да се използва за ограничаване на общата мощност (за удължени периоди) до
# вентилатора. Ако тази стойност е по-малко от 1.0 тогава заявки за скорост на вентилатора
# ще бъде мащабиран между нула и max_power (например, ако
# max_power е .9 и се иска скорост на вентилатора от 80% тогава вентилаторът
# мощността ще бъде настроена на 72%). По подразбиране е 1.0.
#shutdown_speed: 0
# Желаната скорост на вентилатора (изразена като стойност от 0,0 до 1,0), ако
# софтуерът на микро-контролера влиза в състояние на грешка. По подразбиране
# е 0.
#cycle_time: 0.010
# Количеството време (за секунди) за всеки цикъл на pWM мощност към
# фен. Препоръчва се това да бъде 10 милисекунди или по-голямо, когато
# използване на софтуер, базиран PWM. По подразбиране е 0.010 секунди.
#hardware_pwm: Невярно
# Активирайте това, за да използвате хардуер pWM вместо софтуер PWM. Повечето фенове
# не работят добре с хардуер PWM, така че не се препоръчва да се
# активирайте това, освен ако няма електрическо изискване за превключване на
# много високи скорости. При използване на хардуер pWM действителното време цикъл е
# ограничен от изпълнението и може да бъде значително
# по-различно от исканото cycle_time. По подразбиране е False.
#kick_start_time: 0.100
# Време (в секунди), за да стартирате вентилатора на пълна скорост, когато или първи
# разрешаване или увеличаването му с повече от 50% (помага за получаване на вентилатора
# предене). По подразбиране е 0.100 секунди.
#off_below: 0.0
# Минималната входна скорост, която ще захранва вентилатора (изразена като
# стойност от 0,0 до 1,0). Когато скорост, по-ниска от off_below, е
# поиска вентилатора вместо това ще бъде изключен. Тази настройка може да е
# използва за предотвратяване на вентилаторни сергии и за да се гарантира ритник започва са
# ефективно. По подразбиране е 0.0.
#
# Тази настройка трябва да се пренастроява винаги, когато max_power се регулира.
# За да калибрирате тази настройка, започнете с off_below настроен на 0,0 и
# предене на вентилатора. Постепенно понижавайте скоростта на вентилатора, за да определите най-ниската
# входна скорост, която надеждно задвижва вентилатора без сергии. Множество
# off_below на цикъла на митото, съответстващ на тази стойност (за
# пример, 12% -> 0.12) или малко по-висока.
#tachometer_pin:
# Тахометър входен щифт за наблюдение на скоростта на вентилатора. А pullup е като цяло
# изисква се. Този параметър не е задължителен.
#tachometer_ppr: 2
# Когато е посочено tachometer_pin, това е броят на импулсите на
# революция на сигнала на тахометъра. За вентилатор на BLDC това е
# нормално половината от броя на полюсите. По подразбиране е 2.
#tachometer_poll_interval: 0.0015
# Когато е посочено tachometer_pin, това е периодът на гласуване на
# тахометър щифт, за секунди. По подразбиране е 0.0015, което е бързо
# достатъчно за фенове под 10000 RPM при 2 PPR. Това трябва да е по-малко от
# 30 /(tachometer_ppr*rpm), с известен марж, където rpm е
# максимална скорост (в RPM) на вентилатора.
```
...
#tachometer_ppr:
#tachometer_poll_interval:
# Вижте раздела "Вентилатор" за описание на горните параметри.
#fan_speed: 1.0
# Скоростта на вентилатора (изразена като стойност от 0,0 до 1,0), която вентилаторът
# ще бъде настроен, когато е активен нагревател или стъпков драйвер.
# По подразбиране е 1,0
#idle_timeout:
# Времето (в секунди), след като стъпков драйвер или нагревател
# е бил активен и вентилаторът трябва да продължи да работи. По подразбиране
# е 30 секунди.
#idle_speed:
# Скоростта на вентилатора (изразена като стойност от 0,0 до 1,0), която вентилаторът
# ще бъде настроен, когато нагревателят или стъпковият драйвер са били активни и
# преди да бъде достигнат idle_timeout. По подразбиране е fan_speed.
#нагревател:
#степер:
# Име на секцията от конфигурацията, определяща нагревателя/степера, който този вентилатор
# е свързан с него. Ако е списък от имена на нагреватели/степери, разделени със запетая
# е предоставен тук, тогава вентилаторът ще бъде активиран, когато някой от дадените
# нагреватели/степери са активирани. По подразбиране нагревателят е "екструдер", а
# стъпка по подразбиране са всички от тях.
```
...
[дисплей]
lcd_type:
# Задава се на "ssd1306" или "sh1106" за дадения тип дисплей.
#i2c_mcu:
#i2c_bus:
#i2c_speed:
# Незадължителни параметри, достъпни за дисплеи, свързани чрез i2c
# шина. Вижте раздела "Общи I2C настройки" за описание на
# горните параметри.
#cs_pin:
#dc_pin:
#spi_speed:
...
#spi_software_mosi_pin:
#spi_software_miso_pin:
# Изводите, свързани към дисплея, когато е в "4-проводен" spi режим. Вижте
# "общи настройки на SPI" за описание на параметрите.
# които започват с "spi_". По подразбиране се използва режим i2c за
# дисплея.
#reset_pin:
# На дисплея може да се посочи извод за нулиране. Ако той не е
# посочен, тогава хардуерът трябва да има издърпване на
# съответната линия на дисплея.
#contrast:
# Контрастът, който трябва да се зададе. Стойността може да варира от 0 до 256 и
# по подразбиране е 239.
#vcomh: 0
# Задайте стойността на Vcomh на дисплея. Тази стойност е свързана с
# ефект на "размазване" при някои OLED дисплеи. Стойността може да варира
# от 0 до 63. По подразбиране е 0.
# инвертиране: False
# TRUE инвертира пикселите на някои OLED дисплеи.  По подразбиране е
# False.
#x_offset: 0
# Задаване на стойността на хоризонталното отместване при дисплеи SH1106. По подразбиране е
# 0.
...
```
