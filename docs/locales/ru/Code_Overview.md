...
* Не храните ссылку на объект `config` в переменной-члене класса (или в любом другом месте, которое может сохраняться после первоначальной загрузки модуля). Объект `config` является ссылкой на класс «фазы загрузки конфигурации», и вызов его методов после завершения «фазы загрузки конфигурации» не является допустимым.
...
1. Другие методы. Реализуйте методы `check_move()`, `get_status()`, `get_steppers()`, `home()`, `clear_homing_state()` и `set_position()`. Эти функции обычно используются для обеспечения кинематических проверок. Однако в начале разработки здесь можно использовать шаблонный код.
