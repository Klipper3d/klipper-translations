...
Этот документ является справочником по для использования опций, доступных в файле конфигурации Klipper.
...
Поддержка акселерометров LIS3DH.
...
Инструмент для компенсации неточных показаний зонда из-за поворота в X или Y. См. [Axis Twist Compensation Guide] (Axis_Twist_Compensation.md) для более подробной информации о симптомах, конфигурации и настройке.
...
Информация о настройке дисплея aip31068_spi - очень похожая на hd44780_spi 20x04 (20 символов на 4 строки) с немного другим внутренним протоколом.
...
Магнитный угловой датчик поддержки для чтения степенных измерений угла поворота двигателя с помощью чипов SPI 1333, 1337d, mt6816, mt6826 или tle5012b. Измерения доступны через [API Server] (API_Server.md) и [инструмент анализа эмоций](Debugging.md#motion-analysis-and-data-logging). [G-Code reference](G-Codes.md#angle) для доступных команд.
...
Большинство микроконтроллеров Klipper поддерживают только `i2c_speed` 10000 (* стандартный режим*, 100kbit/s). Микроконтроллер Klipper «Linux» поддерживает скорость 40000 (*быстрый режим *, 400кбит/с), но он должен быть [установлен в операционной системе](RPi_microcontroller.md# Опционно-поддерживающий i2c) и параметр `i2c_speed` в противном случае игнорируется. Микроконтроллер Klipper «RP2040» и семейство ATmega AVR и некоторые STM32 (F0, G0, G4, L4, F7, H7) поддерживают скорость 40000 через параметр `i2c_speed`. Все остальные микроконтроллеры Klipper используют скорость 100000 и игнорируют параметр `i2c_speed`.
