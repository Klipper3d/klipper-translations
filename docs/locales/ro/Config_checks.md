...
Acest document oferă o listă de pași pentru a confirma setările pinilor din fișierul printer.cfg. Ar fi o idee bună să efectuezi acești pași după urmarea celor din [installation document](Installation.md).

Pe parcursul acestui ghid, ar putea fi necesar să modifici fișierul config pentru Klipper. Asigură-te că faci un RESTART după fiecare schimbare la fișierul config pentru a te asigura că schimbarea are efect. (scrie ”restart” în terminalul Octoprint și apoi apasă ”Trimite”). E o idee bună să execuți comanda STATUS după fiecare RESTART pentru a verifica dacă fișierul config a fost încărcat cu succes.
...
## Verifică elementele de încălzire
...
Dacă imprimanta are un pat de printare încălzit, atunci execută testul de mai sus cu patul de printare.

## Verifică pinul de activare al motorului cu pas
...
## Verifică endstop-urile

Manually move all the printer axes so that none of them are in contact with an endstop. Send a QUERY_ENDSTOPS command via the command console. It should respond with the current state of all of the configured endstops and they should all report a state of "open". For each of the endstops, rerun the QUERY_ENDSTOPS command while manually triggering the endstop. The QUERY_ENDSTOPS command should report the endstop as "TRIGGERED".

If the endstop appears inverted (it reports "open" when triggered and vice-versa) then add a "!" to the pin definition (for example, "endstop_pin: ^PA2"), or remove the "!" if there is already one present.

Dacă endstop-ul nu se schimbă deloc, acest lucru înseamnă că endstop-ul este conectat la un pin diferit. Totuși, s-ar putea să fie nevoie să schimbi și setarea de pullup a pin-ului (acel '^' la începutului numelui endstop_pin - majoritatea imprimantelor folosesc o rezistență pullup și acel '^' ar trebui să fie prezent).

## Verifică motoarele cu pas
