...
- `SYNC_STEPPER_TO_EXTRUDER STEPPER=<名称> [EXTRUDER=<名称>]`：该命令将使给定挤出机的 STEPPER（如在[extruder](Config_Reference#extruder)或[extruder stepper](Config_Reference#extruder_stepper)配置分段中指定）和指定的 EXTRUDER 同步。如果 EXTRUDER 是一个空字符串，那么步进器将不会被同步到挤出机。
...
当 [tsl1401cl 耗材宽度传感器配置分段](Config_Reference.md#tsl1401cl_filament_width_sensor)或 [霍尔耗材宽度传感器配置分段](Config_Reference.md#hall_filament_width_sensor)被启用以下命令可用（另见[TSLl401CL长丝宽度传感器](TSL1401CL_Filament_Width_Sensor.md)和[霍尔长丝宽度传感器](Hall_Filament_Width_Sensor.md)：

- `QUERY_FILAMENT_WIDTH` - 返回当前测量的耗材宽度
- `RESET_FILAMENT_WIDTH_SENSOR` - 清除所有传感器读数。在更换耗材后可以使用
- `DISABLE_FILAMENT_WIDTH_SENSOR` - 关闭耗材直径传感器并停止使用它进行流量控制。
- `ENABLE_FILAMENT_WIDTH_SENSOR` - 启用耗材宽度传感器，并开始使用它进行流量控制。

### 霍尔耗材宽度传感器命令

当[霍尔耗材宽度传感器配置分段](Config_Reference.md#hall_filament_width_sensor)被启用时，以下命令可用：

- `QUERY_RAW_FILAMENT_WIDTH` - 返回当前 ADC 通道的读数和校准点的原始传感器值。
- `ENABLE_FILAMENT_WIDTH_LOG` - 打开直径记录
- `DISABLE_FILAMENT_WIDTH_LOG` - 关闭耗材直径记录
