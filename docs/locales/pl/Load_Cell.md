...
W tym dokumencie opisano obsługę czujników tensometrycznych w systemie Klipper. Podstawowe funkcje czujników tensometrycznych mogą służyć do odczytu danych siły i ważenia przedmiotów, takich jak filament. Skalibrowany czujnik siły jest ważną częścią sondy opartej na czujnikach tensometrycznych.
...
## Użyj  `LOAD_CELL_DIAGNOSTIC`

Po pierwszym podłączeniu ogniwa wagowego warto sprawdzić, czy nie występują jakieś problemy, uruchamiając narzędzie `LOAD_CELL_DIAGNOSTIC`. To narzędzie zbiera dane z ogniwa wagowego przez 10 sekund i generuje statystyki:
...
Rzeczy, które możesz sprawdzić przy pomocy tych danych:

* Skonfigurowana częstotliwość próbkowania czujnika powinna być zbliżona do wartości „Liczba próbek zmierzonych na sekundę”. Jeśli tak nie jest, może to oznaczać problem z konfiguracją lub okablowaniem.
* „Próbki nasycone” powinny być równe 0. Jeżeli próbki są nasycone, oznacza to, że obciążenie jest większe, niż jest w stanie zmierzyć.
* „Unikalne wartości” powinny stanowić duży procent wartości „Zebranych próbek”. Jeśli „Unikalne wartości” wynoszą 1, najprawdopodobniej jest to problem z okablowaniem.
...
## Kalibracja ogniwa wagowego

Kalibracja ogniw wagowych odbywa się za pomocą polecenia `LOAD_CELL_CALIBRATE`. To interaktywne narzędzie do kalibracji prowadzi użytkownika przez 3-etapowy proces:

1. Najpierw użyj polecenia `TARE`, aby ustalić wartość siły zerowej. To jest wartość konfiguracyjna `reference_tare_counts`.
