...
* Wysoka precyzja ruchu krokowego. Klipper wykorzystuje procesor aplikacyjny (taki jak tani Raspberry Pi) podczas obliczania ruchów drukarki. Procesor aplikacji określa, kiedy do kroku każdego silnika krokowego kompresuje te zdarzenia, przekazuje je do mikrokontrolera, a następnie mikrokontroler wykonuje każde zdarzenie w żądanym czasie. Każde zdarzenie krokowe jest zaplanowane z dokładnością do 25 mikrosekund lub lepszą. Oprogramowanie nie wykorzystuje szacunków kinematycznych (takich jak algorytm Bresenhama) - zamiast tego oblicza precyzyjne czasy kroków w oparciu o fizykę przyspieszenia i fizykę kinematyki maszyny. Bardziej precyzyjny ruch krokowy przekłada się na cichszą i bardziej stabilną pracę drukarki.
