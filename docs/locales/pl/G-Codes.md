...
`ANGLE_CHIP_CALIBRATE CHIP=<chip_name>`:Wykonaj kalibracje wewnętrznego czujnika, jeśli jest zaimplementowany (MT6826S/MT6835).
...
- **AXIS:** Zdefiniuj oś (`X` lub `Y`) dla której zostanie skalibrowana kompensacja skręcenia. Jeśli nie zostanie podana, domyślną osią jest `'X'`
...
- Wybór  płaszczyzny  zakrzywienia: G17 (płaszczyzna XY ), G18 (płaszczyzna XZ), G19 (płaszczyzna YZ)
