...
این سند یک نمای کلی از نحوه پیاده سازی حرکت ربات توسط کلیپر ارائه می دهد ([kinematics] (https://en.wikipedia.org/wiki/Kinematics)). محتویات ممکن است برای توسعه دهندگان علاقه مند به کار بر روی نرم افزار Klipper و همچنین کاربران علاقه مند به درک بهتر مکانیک ماشین های خود جالب باشد.
...
سیستم "پیشرفت فشار" تلاش می کند تا با استفاده از یک مدل متفاوت برای اکسترودر، این را توضیح دهد. به جای اینکه ساده لوحانه باور کند که هر میلی متر^3 فیلامنت وارد شده به اکسترودر منجر به خروج آن مقدار mm^3 بلافاصله از اکسترودر می شود، از مدلی بر اساس فشار استفاده می کند. هنگامی که فیلامنت به داخل اکسترودر فشار داده می‌شود، فشار افزایش می‌یابد (مثل [قانون هوک] (https://en.wikipedia.org/wiki/Hooke%27s_law)) و فشار لازم برای اکسترود تحت تسلط سرعت جریان از طریق دهانه نازل است (مانند قانون [Poiseuille's). قانون] (https://en.wikipedia.org/wiki/Poiseuille_law)). ایده اصلی این است که رابطه بین رشته، فشار و سرعت جریان را می توان با استفاده از یک ضریب خطی مدل کرد:
