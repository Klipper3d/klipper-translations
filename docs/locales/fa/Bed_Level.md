...
تراز کردن صفحه‌ی چاپ (که گاهی از آن با عنوان «Bed Tramming» نیز یاد می‌شود) برای دستیابی به چاپ‌های باکیفیت بسیار حیاتی است.
اگر صفحه به‌درستی «تراز» نشده باشد، ممکن است باعث چسبندگی ضعیف لایه‌ی اول به صفحه، تاب‌برداشتن قطعه (Warping)، و بروز مشکلات ظریف در سراسر چاپ شود.
این سند به‌عنوان راهنمای انجام عملیات تراز کردن صفحه در Klipper تهیه شده است.

درک هدف تسطیح تخت بسیار مهم است. اگر در حین پرینت به چاپگر دستور داده شود در موقعیت "X0 Y0 Z10" قرار گیرد، هدف این است که نازل چاپگر دقیقاً 10 میلی متر از بستر چاپگر فاصله داشته باشد. علاوه بر این، اگر به چاپگر دستور داده شود در موقعیت «X50 Z10» قرار گیرد، هدف این است که نازل در طول کل حرکت افقی فاصله دقیق 10 میلی متری را از بستر حفظ کند.

برای به دست آوردن کیفیت چاپ چاپگر باید کالیبره شود تا فاصله های Z در حدود 25 میکرون (.025mm) دقیق باشد. این یک فاصله کوچک است - به طور قابل توجهی کوچکتر از عرض یک موی معمولی انسان است. این مقیاس را نمی توان با چشم اندازه گیری کرد. اثرات جانبی (مانند گسترش گرما) در این مقیاس اندازه گیری می کند. راز دستیابی به دقت بالا، استفاده از یک فرآیند تکراری و استفاده از یک روش سطح بندی است که دقت بالایی از سیستم حرکت خود پرینتر را به خود جلب می کند.

## مکانیسم کالیبراسیون مناسب را انتخاب کنید

انواع مختلف پرینترها از روش های مختلف برای انجام سطح تخت استفاده می کنند. همه آنها در نهایت به "آزمون کاغذ" (که در زیر نوشته شده است) بستگی دارند. با این حال، فرآیند واقعی برای نوع خاصی از پرینتر در سایر اسناد شرح داده شده است.

قبل از اجرای هر یک از این ابزارهای کالیبراسیون، مطمئن شوید که چک های توضیح داده شده در [config check document] (Config_checks.md) را اجرا کنید. قبل از انجام تسطیح بستر لازم است حرکت اصلی چاپگر را بررسی کنید.

برای چاپگرهایی که دارای «پروب Z خودکار» هستند، حتماً پروب را طبق دستورالعمل‌های موجود در سند [Probe Calibrate] (Probe_Calibrate.md) کالیبره کنید. برای چاپگرهای دلتا، به سند [Delta Calibrate] (Delta_Calibrate.md) مراجعه کنید. برای چاپگرهای دارای پیچ تخت و پایانه های Z سنتی، به سند [Manual Level] (Manual_Level.md) مراجعه کنید.
