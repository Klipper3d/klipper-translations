...
این سند، فرمان‌هایی را که کیلیپر پشتیبانی می‌کند، تشریح می‌نماید. این‌ها فرمان‌هایی هستند که می‌توان آن‌ها را در تب ترمینال OctoPrint وارد کرد.

## دستورات G-Code

Klipper از دستورات استاندارد G-Code پشتیبانی می کند:

- حرکت (G0 یا G1): ` G1 [X<pos] [i<pos> [Z<pos>] [F<سرعت> [x67X]
- ساکن: "G4 P< میلی ثانیه>".
- به سمت مبدأ حرکت کنید: G28 [X] [Y] [Z]
- موتور را خاموش کنید : M18 ` یا ` M84 `
- منتظر حرکت فعلی باشید تا به پایان برسد: `M400 `
- استفاده از فاصله های مطلق / نسبی برای اکستروژن: `M82 `، `M83 `
- استفاده از هماهنگی های مطلق / نسبی: ` G90 `، ` G91 `
- تنظیم موقعیت: `G92 [X<pos>] [Y<pos>] [Z<pos>] [E<pos>]`
- تنظیم فاکتور سرعت معکوس درصد: ` M220 S<percent> `
- درصد رد ضریب اکسترود را تنظیم کنید: «M221 S< درصد>».
- شتاب را تنظیم کنید: `M204 S<value> ` یا `M204 P<Value> T<value> `
   - توجه: اگر S مشخص نشود و هر دو P و T مشخص شوند، شتاب به حداقل P و T تنظیم می شود. اگر فقط یکی از P یا T مشخص شود، دستور هیچ تاثیری ندارد.
- دریافت درجه حرارت: `M105 `
- تنظیم دما: `M104 [T<index] [S< Temperature> `
- دمای اکسترودر را تنظیم کنید و منتظر بمانید: `M109 [T<index>] S<temperature>`
