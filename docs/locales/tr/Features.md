...
* Klipper, birden fazla mikro denetleyiciye sahip yazıcıları destekler. Örneğin, bir mikro denetleyici ekstrüderi kontrol ederken, bir diğeri yazıcının ısıtıcılarını, üçüncüsü ise yazıcının geri kalanını kontrol edebilir. Klipper ana yazılımı, mikro denetleyiciler arasındaki saat kaymasını telafi etmek için saat senkronizasyonu uygular. Birden fazla mikro denetleyiciyi etkinleştirmek için özel bir kod gerekmez; yapılandırma dosyasına birkaç ek satır eklemek yeterlidir.
