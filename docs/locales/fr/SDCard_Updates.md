...
Si votre carte est flashée avec un firmware qui se connecte à un taux de transfert personnalisé, il est possible de mettre à jour en spécifiant l'option `-b` :
...
- Bien qu'il soit possible de flasher une image qui change le taux de transfert série (Serial Baud) ou l'interface de connexion (par exemple, de USB à UART), la vérification échouera toujours car le script ne pourra pas se reconnecter au MCU pour vérifier la version actuelle.
- Seules les cartes qui utilisent SPI pour la communication avec la carte SD sont supportées. Les cartes qui utilisent SDIO, comme la Flymaker Flyboard et la MKS Robin Nano V1/V2, ne fonctionneront pas en mode SDIO. Cependant, il est généralement possible de flasher ces cartes en utilisant le mode SPI logiciel. Mais si le bootloader de la carte n'utilise que le mode SDIO pour accéder à la carte SD, un cycle d'alimentation de la carte et de la carte SD sera nécessaire pour que le mode puisse passer de SPI à SDIO pour terminer le flashage. De telles cartes devraient être définies avec `skip_verify` activé pour sauter l'étape de vérification immédiatement après le flashage. Ensuite, après le redémarrage manuel, vous pouvez réexécuter exactement la même commande `./scripts/flash-sdcard.sh`, mais ajouter l'option `-c` pour terminer l'opération de vérification. Voir [Flashing Boards that use SDIO](#flashing-boards-that-use-sdio) pour des exemples.

## Caractéristiques des cartes

Les cartes les plus courantes devraient être disponibles, mais il est possible d'ajouter une nouvelle définition de carte si nécessaire. Les définitions de cartes sont situées dans `~/klipper/scripts/spi_flash/board_defs.py`. Les définitions sont stockées dans un dictionnaire, par exemple :
...
- `firmware_path` : Le chemin de la carte SD où le firmware doit être transféré. La valeur par défaut est `firmware.bin`.
...
- `skip_verify` : Ceci définit une valeur booléenne qui indique aux scripts d'ignorer l'étape de vérification du firmware pendant le processus de flashage. La valeur par défaut est `False`. Il peut être défini à `True` pour les cartes qui nécessitent un cycle d'alimentation manuel pour terminer le flashage. Pour vérifier le firmware par la suite, exécutez à nouveau le script avec l'option `-c` pour effectuer l'étape de vérification. [Voir les avertissements avec les cartes SDIO](#caveats)

Si le logiciel SPI est requis, le champ `spi_bus` doit être réglé sur `swspi` et le champ supplémentaire suivant doit être spécifié :
...
Il devrait être extrêmement rare que le logiciel SPI soit nécessaire, typiquement seules les cartes avec des erreurs de conception ou les cartes qui ne supportent normalement que le mode SDIO pour leur carte SD en auront besoin. La définition de la carte `btt-skr-pro` fournit un exemple du premier cas, et la définition de la carte `btt-octopus-f446-v1` fournit un exemple du seconde cas.

Avant de créer une nouvelle définition de carte, il faut vérifier si une définition de carte existante répond aux critères nécessaires pour la nouvelle carte. Si c'est le cas, un `BOARD_ALIAS` peut être spécifié. Par exemple, l'alias suivant peut être ajouté pour spécifier "ma nouvelle carte" comme alias de "générique-lpc1768" :
...
## Flashage des cartes qui utilisent SDIO
